...
# Function to trigger master orchestrator (ta-main-workflow)
trigger_master_orchestrator() {
  ...
  # Build valid JSON (no inline comments)
  local payload
  payload=$(cat <<JSON
{
...
JSON
  ...
  print_info "Triggering ta-main-workflow with payload below."
  echo "$payload"
  ...
  print_status "Master orchestrator triggered"
}

# Function to trigger individual batch workflow (ta-sub-workflow)
...
trigger_incremental_run() {
  ...
  # Pass both the row cap and explicit batch cap
